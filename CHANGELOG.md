...
- Codex runtime update probes now use a fixed transient executable staging root under `/run`, ignoring caller-controlled `TMPDIR`, while preserving the intentional `noexec` `/tmp` mount, UID/GID `65534` probes, synthetic credential-free state and immutable bundled fallback.
