...
The component files under `third_party/components/` are copied from the corresponding exact upstream tags used by the current image or from stable license files that are unchanged at that tag. `components/codex/SOURCE.env` ties the preserved Codex NOTICE to the selected release, its exact upstream URL and committed Git blob identity. `components/python/SOURCE.env` provides the equivalent binding between the preserved CPython license and the Python version in `mise.lock`. The version-pinning and third-party validation scripts are intended to make omissions or stale records fail CI rather than relying only on this prose.
