...
`.github/workflows/check-upstream.yml` performs scheduled discovery daily at 05:17 UTC and can also be dispatched manually. The read-only job has no persisted checkout credentials. It downloads bounded installer/manifest/archive bytes through strict URL and redirect policies, but treats all of them strictly as data: the installer is not run, the archive is not extracted, and `agy` is not executed. It verifies the installer hash/static manifest contract, the exact manifest schema and SHA-512 archive integrity, then streams the single `antigravity` member to derive the payload SHA-256. Raw proprietary bytes are never retained in workflow artifacts or commits.

If either the installer hash or payload hash differs from the committed reviewed pair, the shared `automation/update-upstreams` PR records the current statically discovered pair. No preliminary execution step is necessary. A human may then dispatch `.github/workflows/review-antigravity-candidate.yml` once with both exact hashes. The credential-free read-only execution job fetches the installer through the same strict downloader, verifies its admitted hash before Bash execution, runs it only in an isolated temporary home, verifies the installed `agy` hash before invoking it, and only then performs bounded `agy --version`/`--help` and compatibility inspection.

The vendor-execution job has no repository write permission. Only schema-validated metadata crosses to the writer job, and the same artifact is validated again after crossing that boundary. Raw vendor stdout/stderr, installer/archive bytes and `agy` bytes are never committed or uploaded as review artifacts. Fresh scheduled discovery invalidates stale candidate/full-review proposals, and newly created candidate state is staged before the updater's no-change decision. The resulting changes share the single human-reviewed automation PR; there is no auto-merge and review status does not gate runtime availability.
