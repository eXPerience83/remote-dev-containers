...
|---|---|---|---|---|---|
| Context7 for Codex | No Context7 runtime/package is retained in the image; device-code onboarding may explicitly download one pinned `ctx7` CLI package into disposable state | Passive status, manual-key/anonymous config lifecycle, update and remove are offline; explicit device login downloads/runs the transient CLI and contacts Context7; explicit `test` and enabled Codex MCP use may contact `mcp.context7.com` | Marked block in Codex `config.toml`; optional API key below persistent `CODEX_HOME`; transient CLI/XDG/npm state is removed after device login | Hosted Streamable HTTP MCP plus optional device-code onboarding; Remote Dev does not run `ctx7 setup` or let the vendor CLI mutate Codex config, skills or `AGENTS.md` | Experimental `dev -> edge` path under #31; not yet a stable-release claim |

Context7 is an external service operated by Upstash and is not part of the Remote Dev image SBOM. The manager refuses to overwrite an unowned Context7 MCP entry, keeps `required = false`, and never prints the API key. Device-code onboarding runs only after explicit user selection, adopts the resulting validated key into the same Codex-private managed state and removes the transient vendor package/login/cache state afterward. See `docs/context7-codex.md` and `docs/context7-codex.es.md` for the lifecycle, privacy/terms boundary and removal contract.
