...
La comprobación definitiva es pegar el texto exacto en una aplicación fuera del navegador. Esta ruta se ha demostrado en Firefox/Windows sobre el despliegue HTTP normal; la matriz más amplia de navegadores/móviles y la UX de selección siguen en #91. HTTP sin cifrar no protege el tráfico del terminal ttyd: un observador en esa ruta puede ver el tráfico del terminal, incluidos los payloads OSC 52, y un atacante activo puede modificarlo. Usa HTTP sólo en una LAN privada de confianza o un túnel protegido; HTTPS es preferible cuando esté disponible. Es una propiedad de toda la conexión HTTP del terminal ttyd, no un riesgo exclusivo de `/copy`. El asset de compatibilidad está ligado deliberadamente a ttyd 1.7.7 y debe reevaluarse en #174 antes de cualquier actualización de ttyd.
