...
If rollback is required, stop Remote Dev before reversing the dataset names. After the retained NFSv4 dataset has been restored to the canonical name, run the same-revision host ACL audit **before** restarting Remote Dev. A normal audit of an NFSv4 rollback source is expected to report NFSv4/non-trivial ACL findings and return non-zero; do not restart the App merely because the rename succeeded. Either keep Remote Dev stopped and repair/migrate the host ACL state back to the Generic/POSIX contract, or explicitly record that the operator is accepting the broader NFSv4 effective-access risk for the rollback period before restarting. Do not merge newer POSIX-side writes back into the NFSv4 backup casually; treat that as a separate data reconciliation task.
