...
Remote Dev exposes the same high-level `autonomous|guarded` approval abstraction used for Codex while retaining provider-specific mechanics. The Antigravity default is `autonomous`; managed autonomous launches add the reviewed launch-scoped vendor `--dangerously-skip-permissions` override, while managed guarded launches omit it and require compatible vendor approval settings. Start and `--continue` share the same resolver, and the menu supports a one-launch override that is consumed once.

Remote Dev does not persist its own approval defaults into Antigravity's vendor settings. Guarded compatibility is checked offline from only the reviewed top-level approval keys; known conflicts fail closed. `remote-dev-doctor` reports that state read-only, and a separate explicit `remote-dev-antigravity-policy repair-guarded --yes` action can remove only known conflicting top-level approval overrides while preserving unrelated settings and fine-grained permission rules.
