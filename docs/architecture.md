...
Remote Dev does not persist its own approval defaults into Antigravity's vendor settings. Guarded compatibility is checked offline from the small reviewed set of top-level fields in `~/.gemini/antigravity-cli/settings.json` that can disable protected behavior. Known globally permissive or unknown relevant values fail closed, while fine-grained `permissions.allow/ask/deny` rules remain user-managed and are not treated as conflicts merely because they exist. `remote-dev-doctor` reports the same state read-only; Remote Dev never rewrites the vendor policy file.
