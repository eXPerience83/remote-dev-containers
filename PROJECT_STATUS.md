...
- Guarded compatibility is diagnosed offline from Antigravity's own `settings.json`. Known globally permissive or unknown relevant values fail closed; fine-grained `allow/ask/deny` rules remain user-managed; Doctor is read-only; Remote Dev does not rewrite the vendor settings file.
