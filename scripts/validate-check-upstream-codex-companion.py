...
def require_exact_active_lines(block: list[str], expected: tuple[str, ...], label: str) -> None:
    normalized = [line.strip() for line in block]
    for command in expected:
        count = normalized.count(command)
        if count != 1:
            raise ValidationError(
                f"{label}: expected exactly one active command {command!r}; found {count}"
            )
...
def validate_text(text: str) -> None:
    ...
    require_exact_active_lines(resolution, RESOLUTION_EXPECTED, "Codex release digest block")
    ...
    require_exact_active_lines(env_updates, ENV_EXPECTED, "Codex versions.env update block")
